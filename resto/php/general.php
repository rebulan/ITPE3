...
function save_daily($dbf)
{
	...
	for ($i=1; $i<=$header['RecordCount']-1; $i++) {
		//print_r($f[$i]);
		//echo "<br>";
		insert('daily_details_test',['location_id'=>0,'daily_forecast_rainfall_percentage'=>$f[$i]['RAINFALLTO'],'rainfall_description'=>$f[$i]['RAINFALLDE'],'cloudcover'=>$f[$i]['CLOUDCOVER'],'humidity'=>$f[$i]['RELHUMIDIT'],'windspeed'=>$f[$i]['WINDSPEED'],'winddirection'=>$f[$i]['WINDDIRECT'],'daily_forecast_low_temp'=>$f[$i]['TEMPMIN'],'daily_forecast_high_temp'=>$f[$i]['TEMPMAX'],'daily_forecast_mean_temp'=>$f[$i]['TEMPMEAN']]);
	}
